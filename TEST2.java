...
public class TEST2 {
	...
	public static void main(String[] args) {
		...
		// Create a player1 boolean that is true if it is player 1's turn and false if
	...
	public static char playerHasWon(char[][] board) {
		...
		for (int i = 0; i < board.length; i++) {
			...
			// EXTENSION: The boolean inARow is true if a player has won by putting n of
			// their chars in row i and false otherwise
			boolean inARow = true;
			// EXTENSION: The char value is one of the chars in row i; we can use this to
			// check if every other char in row i is equal to value
			char value = board[i][0];

			// EXTENSION: First we have to check if the value is not -, because if it is,
			// that means that there is an empty spot in the row so we can automatically say
			// that inARow is false
			if (value == '-') {
				inARow = false;

				// EXTENSION: Otherwise, we can use a nested for loop to check each position in
				// the row starting at index 1 (since index 0 is our value and we don't need to
				// check if board[i][0] equals value) and check if that position equals value
			} else {
				for (int j = 1; j < board[i].length; j++) {
					// EXTENSION: If board[i][j] doesn't equal value, then we know that inARow is
					// false; we can also break out of the nested for loop because we don't need to
					// look at the rest of this row
					if (board[i][j] != value) {
						inARow = false;
						break;
					}
			...
			// EXTENSION: If inARow is true, then we know that each position in row i had a
			// char equal to value which was not a -. In other words, a player has won so we
			// can return value (the char of the winner)
			if (inARow) {
				return value;
			}
		...
		// EXTENSION: Check the diagonal going from upper left to bottom right: [0][0],
		// [1][1], [2][2]...
		...
		// EXTENSION: Check the diagonal going from upper right to bottom left:
		// [0][n-1], [1][n-2], [2][n-3]...
